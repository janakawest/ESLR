...
{
  os << "Destination " <<  m_destination << "/" << m_mask << " Matric " << int (m_matric) << " Tag " << int (m_routeTag) << " Sequence Number "<< int (m_sequenceNumber) << std::endl;
}
...
SRCHeader::SRCHeader(): m_sequenceNumber (0),
                        m_rho (0),
                        m_mue (0),
                        m_lambda (0),
...
{
  os << "Server Address " <<  m_serverAddress << "/" << m_netMask << " Matric " << int (m_rho)  << " Sequence Number "<< int (m_sequenceNumber) << std::endl;
}
...
SRCHeader::Serialize (Buffer::Iterator i) const
{
  ...
  i.WriteHtonU16 (m_mue);
  i.WriteHtonU16 (m_lambda);
}
...
ESLRRoutingHeader::Deserialize (Buffer::Iterator start)
{
  ...
  {
    numberofMessages = (i.GetSize () - ESLR_BASE_SIZE) / SRC_SIZE;
    for (uint8_t n=0; n<numberofMessages; n++)
    {
      SRCHeader src;
