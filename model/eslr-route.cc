...
*/
  ...
  RoutingTableEntry::RoutingTableEntry () : m_sequenceNo (0),
                                              m_metric (0),
                                              m_changed (false),
  ...
  RoutingTable::AddRoute (RoutingTableEntry *routingTableEntry, Time invalidateTime, Time deleteTime, Time settlingTime, eslr::Table table)
  {
    if (table == eslr::MAIN)
    {
      ...
      NS_LOG_FUNCTION ("Added a new Route to Main Table " << routingTableEntry->GetDestNetwork () << "/" << int (routingTableEntry->GetDestNetworkMask ().GetPrefixLength ()));
        ...
        Time delay = invalidateTime + Seconds (m_rng->GetValue (0, 10));
    ...
    else if (table == eslr::BACKUP)
    {
      if (settlingTime.GetSeconds () != 0)
      {
        ...
        EventId moveToMainEvent = Simulator::Schedule (settlingTime, &RoutingTable::MoveToMain, this, route2, invalidateTime, deleteTime, settlingTime);
      ...
      else if (settlingTime.GetSeconds () == 0)
      {
        ...
        {
          //Time delay = invalidateTime + Seconds (m_rng->GetValue (0, 20));
          invalidateEvent = EventId ();
        }
        else
        {
          invalidateEvent = Simulator::Schedule (invalidateTime, &RoutingTable::InvalidateRoute, this, route3, p);
        }
  ...
  RoutingTable::MoveToMain (RoutingTableEntry *routingTableEntry, Time invalidateTime, Time deleteTime, Time settlingTime)
  {
    ...
    for (RoutesI it = m_backupRoutingTable.begin (); it != m_backupRoutingTable.end (); it++)
    {
               ...
               (it->first->GetGateway () != routingTableEntry->GetGateway ()))
      {
        ...
        it->second = Simulator::Schedule (invalidateTime, &RoutingTable::InvalidateRoute, this, it->first, p);
      }    
    } 
  }
  ...
  RoutingTable::AddHostRoute (RoutingTableEntry *routingTableEntry, Time invalidateTime, Time deleteTime, Time settlingTime, eslr::Table table)
  {
    ...
    if (table == eslr::MAIN)
    {
      ...
      else
      {
        ...
        EventId invalidateEvent = Simulator::Schedule (invalidateTime, &RoutingTable::InvalidateRoute, this, route2, p);
    ...
    else if (table == eslr::BACKUP)
    {
      if (settlingTime.GetSeconds () != 0)
      {
        ...
        EventId moveToMainEvent = Simulator::Schedule (settlingTime, &RoutingTable::MoveToMain, this, route4, invalidateTime, deleteTime, settlingTime);
      ...
      else if (settlingTime.GetSeconds () == 0)
      {
        ...
        EventId invalidateEvent = Simulator::Schedule (invalidateTime, &RoutingTable::InvalidateRoute, this, route3, p);
    ...
    if (param.table == eslr::MAIN)
    {
      ...
      if (param.invalidateType == eslr::EXPIRE)
      {
        ...
        if (foundBackupRoute && (mainRoute->first->GetMetric () > secondaryRoute->first->GetMetric ()))
        {
          ...
          Time delay = Seconds (m_rng->GetValue (0, 3));
        ...
        else if (mainRoute->first->GetMetric () < primaryRoute->first->GetMetric ())
        {
          ...
          else
          {
            ...
            mainRoute->second = Simulator::Schedule (param.deleteTime, &RoutingTable::DeleteRoute, this, mainRoute->first, eslr::MAIN);

            primaryRoute->second = Simulator::Schedule (param.deleteTime, &RoutingTable::DeleteRoute, this, primaryRoute->first, eslr::BACKUP);
          }             
          return (retVal = true);
        }
      }
      ...
      if (foundInBackup)
      {
        if (bkupRoute->first->GetRouteType () == eslr::SECONDARY)
        {
          ...
          else
          {
            ...
            bkupRoute->second = Simulator::Schedule (param.deleteTime, &RoutingTable::DeleteRoute, this, bkupRoute->first, eslr::BACKUP);            
          }
        }
  ...
  RoutingTable::UpdateNetworkRoute (RoutingTableEntry *routingTableEntry, Time invalidateTime, Time deleteTime, Time settlingTime, eslr::Table table)
  {
    ...
    if (table == eslr::MAIN)
    {
      ...
      if (foundInMain && (routingTableEntry->GetGateway () != Ipv4Address::GetZero ()))
      {
        ...
        mainRoute->second = Simulator::Schedule (invalidateTime, &RoutingTable::InvalidateRoute, this, mainRoute->first, p);
    ...
    {    
      ...
      {        
        if (foundPrimary)
        {
          ...
          primaryRoute->second = Simulator::Schedule (settlingTime, &RoutingTable::MoveToMain, this, primaryRoute->first, invalidateTime, deleteTime, settlingTime);
      ...
      else if (routingTableEntry->GetRouteType () == eslr::SECONDARY)
      {
        if (foundSecondary)
        {
          ...
          secondaryRoute->second = Simulator::Schedule (invalidateTime, &RoutingTable::InvalidateRoute, this, secondaryRoute->first, p);     
    ...
    if (foundInMain)
    {
      for (RoutesI it = routesInMain.begin ();  it!= routesInMain.end (); it++)
      {
       if (it->second.IsRunning ())
        ...
        Time delay = Seconds (invalidateTime.GetSeconds () / 10) + Seconds (m_rng->GetValue (0, 15));
