...
NS_OBJECT_ENSURE_REGISTERED (EslrRoutingProtocol);
  ...
  TypeId EslrRoutingProtocol::GetTypeId (void)
  {
    static TypeId tid = TypeId ("ns3::eslr::EslrRoutingProtocol")
      ...
      .AddAttribute ( "PrintingMethod", "Specify which table has to be print.",
                      EnumValue (eslr::MAIN_R_TABLE),
                      MakeEnumAccessor (&EslrRoutingProtocol::m_print),
                      MakeEnumChecker ( eslr::MAIN_R_TABLE, "MainRoutingTable",
                                        eslr::N_TABLE, "NeighborTable",
                                        eslr::BACKUP_R_TABLE, "BackupRoutingTable"))
    ;
    return tid;
  }
  ...
  EslrRoutingProtocol::Receive (Ptr<Socket> socket)
  {
    ...
    {
      NS_LOG_LOGIC ("A piggibacked pakcet, Ignoring it.");
      return;
    }

    NS_LOG_LOGIC ("Handle the request packt.");
    ...
    if (hdr.GetCommand () == eslr::KAM)
    {
      ...
      NS_LOG_LOGIC ("Handle the KAM packt.");
  ...
  EslrRoutingProtocol::HandleKamRequests (ESLRRoutingHeader hdr, Ipv4Address senderAddress, uint32_t incomingInterface)
  {
    ...
    if (m_interfaceExclusions.find (incomingInterface) == m_interfaceExclusions.end ())
    {
      ...
      for (std::list<KAMHeader>::iterator iter = kams.begin (); iter != kams.end (); iter++)
      {
        ...
        if (!isNeighborPresent)
        {
          NS_LOG_LOGIC ("Adding the new neighbor record" <<  iter->GetNeighborID () << iter->GetGateway ());

          NeighborTableEntry* newNeighbor = new  NeighborTableEntry ( iter->GetNeighborID (),
                                                                      iter->GetGateway (),
                                                                      iter->GetGatewayMask (),
                                                                      incomingInterface,
                                                                      receivedSocket,
                                                                      ...
                                                                      iter->GetAuthData (),
																																			eslr::VALID);
          m_neighborTable.AddNeighbor ( newNeighbor, 
                                        m_neighborTimeoutDelay, 
				...
				{
          NS_LOG_DEBUG ("Updating the neighbor record" <<  neighborRecord->first->GetNeighborID () << neighborRecord->first->GetNeighborAddress ());
                                                     ...
                                                     neighborRecord->first->GetNeighborID (),
    																								 neighborRecord->first->GetNeighborAddress (), 
																										 ...
																										 neighborRecord->first->GetAuthData (),
						 																				 eslr::VALID);
					m_neighborTable.UpdateNeighbor (existingNeighbor, 
                                          m_neighborTimeoutDelay, 
                                          m_garbageCollectionDelay, 
                                          m_routing);	
  ...
  void EslrRoutingProtocol::sendKams ()
  {
    ...
    // Reschedule next KAM
		Time sendKam = Seconds (m_rng->GetValue (0, 0.05*m_kamTimer.GetSeconds ()));
  ...
  EslrRoutingProtocol::HandleRouteResponses (ESLRRoutingHeader hdr, Ipv4Address senderAddress, uint32_t incomingInterface)
  {
    ...
    NS_LOG_LOGIC ("Invalidating all unresponsive and broken routes.");
    ...
    for (std::list<ESLRrum>::iterator it = rums.begin (); it != rums.end (); it++)
    {
      if ((it->GetSequenceNo () % 2) != 0)
      {
        ...
        if (invalidatedInMain)
        {
          ...
          // As there are invalidated routes, an immidiate triggered update is triggered
          // Note: However, Triggered updates are frequently called, the compansating time (1-5s)
          // is affected
          SendTriggeredRouteUpdate ();
        }
      ...
      else
      {
        ...
        if (!foundPrimary && !foundSecondary)
        {
          ...
          NS_LOG_LOGIC ("New network received and add it to the backup table");
  ...
  {
    NS_LOG_FUNCTION (this << "Find and delete routes for interface: " << destAddress );
  ...
  EslrRoutingProtocol::DoSendRouteUpdate (eslr::UpdateType updateType)
  {
    ...
    else if (updateType == eslr::TRIGGERED)  
    {
      ...
      std::cout << int (m_ipv4->GetObject<Node> ()->GetId ()) << " Triggered Update" << std::endl;
    }
    ...
    for (rtIter = tempMainTable.begin (); rtIter != tempMainTable.end (); rtIter++)
    {
      ...
      {
        NS_LOG_LOGIC (this << "Route is about my local interfaces. Update the Sequence#");
        
        rtIter->first->SetSequenceNo (rtIter->first->GetSequenceNo () + 2);
        rtIter->first->SetRouteChanged (false);
        m_routing.UpdateNetworkRoute (rtIter->first, m_routeTimeoutDelay, m_garbageCollectionDelay, m_routeSettlingDelay, eslr::MAIN);
      }
  ...
  EslrRoutingProtocol::SendTriggeredRouteUpdate ()
  {
    ...
    //        In addition, triggered updates are omited if there is a scheduled periodic update.
    //        Furhtermore, only changed routes will be sent. 
    //        Rest of the reoutes are not considered as a RUM for the update message. 
    //        Thus, aftersending either triggered or periodic update, the changed flag will be set to false 
